...
FROM
    (((((((((`vd_nota_fiscal` `nota_fiscal`
JOIN `vd_nota_fiscal_item` `nota_fiscal_item` ON
    (((`nota_fiscal`.`cod_empresa` = `nota_fiscal_item`.`cod_empresa`)
        AND (`nota_fiscal`.`cod_nota_fiscal` = `nota_fiscal_item`.`cod_nota_fiscal`))))
JOIN `cd_produto` `produto` ON
    ((`nota_fiscal_item`.`cod_produto` = `produto`.`cod_produto`)))
JOIN `cd_cliente_endereco` `cliente_endereco` ON
    ((`nota_fiscal`.`cod_cliente` = `cliente_endereco`.`cod_cliente`)))
JOIN `vd_nota_fiscal_atividade` `nota_fiscal_atividade` ON
    (((`nota_fiscal`.`cod_nota_fiscal` = `nota_fiscal_atividade`.`cod_nota_fiscal`)
        AND (`nota_fiscal_item`.`cod_nota_fiscal` = `nota_fiscal_atividade`.`cod_nota_fiscal`))))
JOIN `cd_ramo_atividade` `ramo_atividade` ON
    ((`nota_fiscal_atividade`.`cod_ramo_atividade` = `ramo_atividade`.`cod_ramo_atividade`)))
JOIN `cd_cliente_atividade` `cliente_atividade` ON
    (((`nota_fiscal`.`cod_cliente` = `cliente_atividade`.`cod_cliente`)
...
JOIN `cd_empresa` `empresa` ON
    (((`nota_fiscal`.`cod_empresa` = `empresa`.`cod_empresa`)
        AND (`nota_fiscal_item`.`cod_empresa` = `empresa`.`cod_empresa`))))
JOIN `sg_colaborador` `vendedor` ON
    ((`nota_fiscal`.`cod_colaborador` = `vendedor`.`cod_colaborador`)))
JOIN `cd_cond_pagto` `cond_pagto` ON
    ((`nota_fiscal`.`cod_cond_pagto` = `cond_pagto`.`cod_cond_pagto`)))
WHERE
    (`nota_fiscal`.`cod_empresa` IN (1, 2, 3, 6, 13))
ORDER BY
